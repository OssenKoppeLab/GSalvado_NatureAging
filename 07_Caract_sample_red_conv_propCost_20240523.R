...
results_print_MTL=data.frame(matrix(nrow = 12,ncol = 18))
...
for (i_method in 1:length(c_method_plasma)) {
  ...
  for (i_method2 in 1:length(c_method_PET)) {
    ...
    if (i_method2==1) {
      ...
      results_print_MTL$USD_neg[i_count]=paste(format(round(((100*n_part*v_cost_plasma)/(sum((df3[which(df3$plasma_pos==0),"conv_MCI"]==1)==T,na.rm = T)))/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),nsmall=1),"%",sep = "")
      results_print_MTL$USD_pos[i_count]=paste(format(round(((100*n_part*v_cost_plasma)/(sum((df3[which(df3$plasma_pos==1),"conv_MCI"]==1)==T,na.rm = T)))/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),nsmall=1),"%",sep = "")
    ...
    results_print_MTL$USD_neg[i_count]=paste(format(round(((100*n_part*v_cost_plasma+n_part*(1-v_perc_PET)*(1-v_perc)*v_cost_PET)/(sum((df3[which(df3$plasma_pos==0 | df3$MTL_pos==0),"conv_MCI"]==1)==T,na.rm = T)))/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),nsmall=1),"%",sep = "")
    results_print_MTL$USD_pos[i_count]=paste(format(round(((100*n_part*v_cost_plasma+n_part*(1-v_perc_PET)*(1-v_perc)*v_cost_PET)/(sum((df3[which(df3$plasma_pos==1 & df3$MTL_pos==1),"conv_MCI"]==1)==T,na.rm = T)))/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),nsmall=1),"%",sep = "")
    ...
    results_plot_MTL$USD[i_count2]=n_part*v_cost_plasma+n_part*(1-v_perc_PET)*(1-v_perc)*v_cost_PET
...
writexl::write_xlsx(results_print_MTL,path = paste(dir_res,"/Charact_sample_red_convMCI_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".xlsx",sep = ""))
writexl::write_xlsx(results_plot_MTL,path = paste(dir_res,"/Plot_charact_sample_red_convMCI_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".xlsx",sep = ""))
...
#   geom_hline(yintercept = n_part*v_cost_PET/172000,linetype="dashed")+#172 number of conversions in the full sample
#   theme_classic()+theme(legend.position = "none",axis.text=element_text(size=18),
#                         axis.title=element_text(size=24,face="bold"),legend.text = element_text(size=6),
#                         axis.line.x = element_line(size = 0.1),axis.line.y = element_line(size = 0.1),
#                         axis.ticks.x=element_line(size = 0.1),axis.ticks.y = element_line(size = 0.1),
...
p4=ggplot(data=results_plot_MTL2,aes(x = Group_plasmaPET, y = 100*Conv/N,fill=Group)) +
  ...
  ylab("Progressed MCI (%)") + xlab("Sample reduction groups")+
  scale_fill_manual(values = c("#d3d3d3",v_color))+
  geom_text(aes(label=paste(round(100*Conv/N,0),"%",sep = ""),y=5+100*Conv/N),position = position_dodge(.9),size=3)+
  theme_classic()+theme(legend.position = "none",axis.text=element_text(size=8),#axis.text.x = element_text(angle=45),
                        axis.title=element_text(size=10,face="bold"),legend.text = element_text(size=6),
                        axis.line.x = element_line(size = 0.1),axis.line.y = element_line(size = 0.1),
                        axis.ticks.x=element_line(size = 0.1),axis.ticks.y = element_line(size = 0.1),
                        legend.key.size = unit(0.2, 'cm'),legend.title = element_blank())+scale_y_continuous(limits=c(0,50))


pcost=ggplot(data=results_plot_MTL2[which(results_plot_MTL2$Group=="Pos"),],aes(x = Group_plasmaPET, y = 100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1)),fill=Group)) +
  geom_bar(stat="identity",position="dodge")+theme_classic() +#scale_fill_manual(values=c())+
  # geom_errorbar(aes(ymin=l_Pos, ymax=h_Pos), width=.2,
  #               position=position_dodge(.9))+
  ylab("USD/conv (%)") + xlab("Sample reduction groups")+
  scale_fill_manual(values = c(v_color))+
  geom_text(aes(label=paste(round(100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1)),1),"%",sep = ""),y=5+100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),position = position_dodge(.9),size=3)+
...
pl2=ggarrange(p4,p1,p0,pcost,#plines_part_CSF,
              ...
              ncol = 3, nrow = 3)
c_out=c_out_name
ggexport(pl2,filename=paste(dir_fig,"/Barplots_charact_sample_red_conv_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change

pl3=ggarrange(pcost,#plines_part_CSF,
              # labels = c("A", "B", "C","D","E"),
              ncol = 3, nrow = 3)
c_out=c_out_name
# ggexport(pl3,filename=paste(dir_fig,"/Barplots_cost_sample_red_conv_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change

pcost2=ggplot(data=results_plot_MTL2[which(results_plot_MTL2$Group=="Pos"),],aes(x = Group_plasmaPET, y = 100-100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1)),fill=Group)) +
  geom_bar(stat="identity",position="dodge")+theme_classic() +#scale_fill_manual(values=c())+
  # geom_errorbar(aes(ymin=l_Pos, ymax=h_Pos), width=.2,
  #               position=position_dodge(.9))+
  ylab("US$ saved (%)") + xlab("Sample reduction groups")+
  scale_fill_manual(values = c(v_color))+
  geom_text(aes(label=paste(round(100-100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1)),1),"%",sep = ""),y=5+100-100*(USD/Conv)/((n_part*v_cost_PET)/sum(df3$conv_MCI==1))),position = position_dodge(.9),size=3)+
  geom_hline(yintercept = 100,linetype="dashed")+#172 number of conversions in the full sample
  theme_classic()+theme(legend.position = "none",axis.text=element_text(size=8),#axis.text.x = element_text(angle=45),
                        axis.title=element_text(size=10,face="bold"),legend.text = element_text(size=6),
                        axis.line.x = element_line(size = 0.1),axis.line.y = element_line(size = 0.1),
...
pl4=ggarrange(pcost2,#plines_part_CSF,
              ...
              ncol = 4, nrow = 4)
c_out=c_out_name
ggexport(pl4,filename=paste(dir_fig,"/Barplots_Savedcost_sample_red_conv_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change

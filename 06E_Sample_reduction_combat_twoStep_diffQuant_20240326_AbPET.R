...
# Step with Abpet after plasma and before Tau-PET
library(longpower)
# 
...
f_N <- function(data, indices,c_out0,v_time0,v_time_int0) {
  ...
  N_or=lmmpower(mdl, pct.change = v_change, t = seq(0,v_time0,v_time_int0), power = v_power)
  
  mdl_plasma=lmer(fml,d[which(d$plasma_pos==1),])
  N_plasma=lmmpower(mdl_plasma, pct.change = v_change, t = seq(0,v_time0,v_time_int0), power = v_power)
  
  #Ab-PET
  mdl_MTL=lmer(fml,d[which(d$plasma_pos==1 & d$ab_pos==1),],REML = F)
  N_MTL=lmmpower(mdl_MTL, pct.change = v_change, t = seq(0,v_time0,v_time_int0), power = v_power)
  # 
  # mdl_NeoT=lmer(fml,d[which(d$NeoT_pos==1),],REML = F)
  # N_NeoT=lmmpower(mdl_NeoT, pct.change = v_change, t = seq(0,4,1), power = v_power)
  
  mdl_MTL_plasma=lmer(fml,d[which(d$MTL_pos==1 & d$plasma_pos==1 & d$ab_pos==1),],REML = F)
  N_MTL_plasma=lmmpower(mdl_MTL_plasma, pct.change = v_change, t = seq(0,v_time0,v_time_int0), power = v_power)
  
  mdl_NeoT_plasma=lmer(fml,d[which(d$NeoT_pos==1 & d$plasma_pos==1 & d$ab_pos==1),],REML = F)
  N_NeoT_plasma=lmmpower(mdl_NeoT_plasma, pct.change = v_change, t = seq(0,v_time0,v_time_int0), power = v_power)
  
  p_plasma0=(N_or$N-N_plasma$N)
  p_MTL0=(N_or$N-N_MTL$N)#Step with Abpet after plasma and before Tau-PET
  ...
  return(c(N_or$N,N_plasma$N,
           ...
           #mod:
           100*N_MTL_plasma$N/N_MTL$N,100*N_NeoT_plasma$N/N_MTL$N,
           #New
           N_MTL$N,p_MTL0,p_MTL_plasma,p_MTL_MTLplasma,100*N_MTL$N/N_plasma$N,100*N_MTL$N/N_or$N))
}
dataAll$cohort=as.factor(dataAll$cohort)
dataBl=dataAll %>% filter(!duplicated(sid))
...
for (i_method in 1:length(c_method_plasma)) {#
  ...
  results_print=data.frame(matrix(nrow = n_out,ncol = 16+6))
  ...
  for (i_out in 1) {#:n_out
    ...
    df$plasma_pos=as.factor(ifelse(df[,c_pl]>quantile(df[which(!duplicated(df$sid)),c_pl],v_perc),1,0))#as.factor(ifelse(dataAll$z_ptau217_comb>1.96,1,0))
    ...
    mdl_plasma=lmer(fml,df[which(df$plasma_pos==1),],REML = F)
    N_plasma=lmmpower(mdl_plasma, pct.change = v_change, t = seq(0,v_time_final,v_time_int), power = v_power)
    
    mdl_MTL=lmer(fml,df[which(df$plasma_pos==1 & df$ab_pos==1),],REML = F)
    N_MTL=lmmpower(mdl_MTL, pct.change = v_change, t = seq(0,v_time_final,v_time_int), power = v_power)
    # 
    # mdl_NeoT=lmer(fml,df[which(df$NeoT_pos==1),],REML = F)
    # N_NeoT=lmmpower(mdl_NeoT, pct.change = v_change, t = seq(0,4,1), power = v_power)
    
    mdl_MTL_plasma=lmer(fml,df[which(df$MTL_pos==1 & df$plasma_pos==1 & df$ab_pos==1),],REML = F)
    N_MTL_plasma=lmmpower(mdl_MTL_plasma, pct.change = v_change, t = seq(0,v_time_final,v_time_int), power = v_power)
    
    mdl_NeoT_plasma=lmer(fml,df[which(df$NeoT_pos==1 & df$plasma_pos==1 & df$ab_pos==1),],REML = F)
    ...
    boot_results= boot(data = df, statistic = f_N, R = n_boot,
                       c_out0=c_out[i_out],v_time0=v_time_final,v_time_int0=v_time_int,strata = df$plasma_pos)#,seed=12345
    ...
    p_perc_MTL0 =(1 + sum(boot_results$t[,17] <= 0)) / (nrow(boot_results$t)+1)
    p_perc_MTL_plasma =(1 + sum(boot_results$t[,18] <= 0)) / (nrow(boot_results$t)+1)
    p_perc_MTL_MTLplasma =(1 + sum(boot_results$t[,19] <= 0)) / (nrow(boot_results$t)+1)
    ci_perc_Ab_plasma=boot.ci(boot_results, type = "norm",seed=12345,index = 20)#Compared to plasma+
    ci_perc_Ab0=boot.ci(boot_results, type = "norm",seed=12345,index = 21)#Compared to plasma+
    ...
    # p_perc_MTL_NeoT =(1 + sum(boot_results$t[,19] <= 0)) / (nrow(boot_results$t)+1)
   ...
   results_print$perc_MTL[i_out]=paste(round(100*N_MTL$N/N_plasma$N,0),"[",round(ci_perc_Ab_plasma[["normal"]][2]),", ",round(ci_perc_Ab_plasma[["normal"]][3]),"]",sep = "")
   results_print$perc_MTL0[i_out]=paste(round(100*N_MTL$N/N_or$N,0),"[",round(ci_perc_Ab0[["normal"]][2]),", ",round(ci_perc_Ab0[["normal"]][3]),"]",sep = "")
   ...
   results_print$perc_plasmaMTL[i_out]=paste(round(100*N_MTL_plasma$N/N_or$N,0),"[",round(ci_perc_pMTL0[["normal"]][2]),", ",round(ci_perc_pMTL0[["normal"]][3]),"]",sep = "")#Compared to initial participants
   results_print$perc_plasmaNeoT[i_out]=paste(round(100*N_NeoT_plasma$N/N_or$N,0),"[",round(ci_perc_pNeoT0[["normal"]][2]),", ",round(ci_perc_pNeoT0[["normal"]][3]),"]",sep = "")#Compared to initial participants
   results_print$perc_plasmaMTL_plasma[i_out]=paste(round(100*N_MTL_plasma$N/N_MTL$N,0),"[",round(ci_perc_pMTL_plasma[["normal"]][2]),", ",round(ci_perc_pMTL_plasma[["normal"]][3]),"]",sep = "")#Compared to Ab-PET positive
   results_print$perc_plasmaNeoT_plasma[i_out]=paste(round(100*N_NeoT_plasma$N/N_MTL$N,0),"[",round(ci_perc_pNeoT_plasma[["normal"]][2]),", ",round(ci_perc_pNeoT_plasma[["normal"]][3]),"]",sep = "")#Compared to Ab-PET positive
   
   
   # results_plot$Model[1]="Original"
   results_plot$Model[1]="Plasma"
   results_plot$Model[2]="Ab-PET_plasma"
   results_plot$Model[2+1]="Ab-PET"
   # results_plot$Model[3]="NeoT"
   results_plot$Model[3+1]="p-MTL"
   results_plot$Model[4+1]="p-NeoT"
   results_plot$Model[5+1]="p-MTL_plasma"
   results_plot$Model[6+1]="p-NeoT_plasma"
   ...
   results_plot$Perc[2]=100*(N_MTL$N)/N_plasma$N#
   ...
   results_plot$Perc[2+1]=100*(N_MTL$N)/N_or$N#
   results_plot$Perc_l[2+1]=ci_perc_Ab0[["normal"]][2]#Compared to plasma+
   results_plot$Perc_h[2+1]=ci_perc_Ab0[["normal"]][3]#Compared to plasma+
   # 
   ...
   results_plot$Perc[2+2]=100*(N_MTL_plasma$N)/N_or$N#-100*(N_MTL_plasma$N-N_or$N)/N_or$N
   results_plot$Perc_l[2+2]=ci_perc_pMTL0[["normal"]][2]
   results_plot$Perc_h[2+2]=ci_perc_pMTL0[["normal"]][3]
   
   #Mod
   results_plot$Perc[3+2]=100*(N_NeoT_plasma$N)/N_or$N#-100*(N_NeoT_plasma$N-N_or$N)/N_or$N
   results_plot$Perc_l[3+2]=ci_perc_pNeoT0[["normal"]][2]
   results_plot$Perc_h[3+2]=ci_perc_pNeoT0[["normal"]][3]
   
   results_plot$Perc[4+2]=100*(N_MTL_plasma$N)/N_MTL$N#-100*(N_MTL_plasma$N-N_or$N)/N_or$N
   results_plot$Perc_l[4+2]=ci_perc_pMTL_plasma[["normal"]][2]
   results_plot$Perc_h[4+2]=ci_perc_pMTL_plasma[["normal"]][3]
   
   results_plot$Perc[5+2]=100*(N_NeoT_plasma$N)/N_MTL$N#-100*(N_NeoT_plasma$N-N_or$N)/N_or$N
   ...
   results_line$N=c(100,100*N_plasma$N/N_or$N,100*N_MTL$N/N_or$N,100*N_MTL_plasma$N/N_or$N,100,100*N_plasma$N/N_or$N,100*N_MTL$N/N_or$N,100*N_NeoT_plasma$N/N_or$N)
   ...
   writexl::write_xlsx(results_plot,path = paste(dir_res,"/Plot_sampleRed_",c_out[i_out],"_Plasma_",c_method_plasma[i_method],"_PET_",c_method_PET[i_method2],"_Ab_",l_ab,"_adj_",l_adj,"_power_",v_power,"_change_",v_change,"_proportion_time_",v_time_final,"_interval_",v_time_int,".xlsx",sep = ""))
   
  }
  
  writexl::write_xlsx(results_print,path = paste(dir_res,"/SampleRed_Plasma_",c_method_plasma[i_method],"_PET_",c_method_PET[i_method2],"_Ab_",l_ab,"_adj_",l_adj,"_power_",v_power,"_change_",v_change,"_proportion_time_",v_time_final,"_interval_",v_time_int,".xlsx",sep = ""))
  }
}

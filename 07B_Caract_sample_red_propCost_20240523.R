...
results_print_MTL=data.frame(matrix(nrow = 12,ncol = 17))
colnames(results_print_MTL)=c("Group_plasma","Group_PET","N_neg","N_pos","Ab_neg","Ab_pos","slope_neg","slope_pos",
                              "APOE_neg","APOE_pos","Fem_neg","Fem_pos","Age_neg","Age_pos","Edu_neg","Edu_pos","USD_pos")

results_plot_MTL=data.frame(matrix(nrow = 2*12,ncol = 13))
colnames(results_plot_MTL)=c("Group_plasmaPET","Group","N","Ab",
                             "APOE","Fem","slope","slope_SD","Age","Age_SD","Edu","Edu_SD","USD")
...
results_print_MTL$USD_pos=paste(format(round(100*((results_plot_MTL_pos$USD)/results_plot_MTL_pos$slope)/((v_cost_PET*n_part)/(mean(df3$slope_cog))),1),nsmall=1)," (",
                                format(round(100*((results_plot_MTL_pos$USD)/(results_plot_MTL_pos$slope-results_plot_MTL_pos$slope_SD/sqrt(results_plot_MTL_pos$N)))/((v_cost_PET*n_part/(mean(df3$slope_cog)-sd(df3$slope_cog)/sqrt(n_part)))),1),nsmall=1),",",
                                format(round(100*((results_plot_MTL_pos$USD)/(results_plot_MTL_pos$slope-results_plot_MTL_pos$slope_SD/sqrt(results_plot_MTL_pos$N)))/((v_cost_PET*n_part/(mean(df3$slope_cog)+sd(df3$slope_cog)/sqrt(n_part)))),1),nsmall=1),")",sep = "")


writexl::write_xlsx(results_print_MTL,path = paste(dir_res,"/Charact_sample_red_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".xlsx",sep = ""))
writexl::write_xlsx(results_plot_MTL,path = paste(dir_res,"/Plot_charact_sample_red_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".xlsx",sep = ""))


results_plot_MTL=readxl::read_xlsx(paste(dir_res,"/Plot_charact_sample_red_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".xlsx",sep = ""))
...
# p0=ggplot(data=results_plot_MTL2,aes(x = Group_plasmaPET, y = 100*N/1440,fill=Group)) +
...
#   geom_text(aes(label=paste(round(100*N/1440,1),"%",sep = "")),position = "dodge",size=6)+
...
# 
# ggexport(p0,filename=paste(dir_fig,"/Barplots_charact_sample_red_Npos_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
#   scale_y_continuous(limits=c(0,100))
# 
# ggexport(p1,filename=paste(dir_fig,"/Barplots_charact_sample_red_Abpos_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
#   scale_y_continuous(limits=c(0,100))
# 
# ggexport(p2,filename=paste(dir_fig,"/Barplots_charact_sample_red_APOE_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
#   scale_y_continuous(limits=c(0,100))
# 
# ggexport(p3,filename=paste(dir_fig,"/Barplots_charact_sample_red_sex_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
# 
...
# # scale_y_continuous(limits=c(0,100))
# 
# ggexport(p4,filename=paste(dir_fig,"/Barplots_charact_sample_red_mPACCslope_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
#   geom_hline(yintercept = abs(mean(df3$slope_cog))/(1440*v_cost_PET/1000000),linetype="dashed")+#172 number of conversions in the full sample
#   #  geom_text(aes(label=paste(round(100*APOE/N,1),"%",sep = "")),position = "dodge",size=12)+
#   theme_classic()+theme(legend.position = "none",axis.text=element_text(size=18),#axis.text.x = element_text(angle=45),
#                         axis.title=element_text(size=24,face="bold"),legend.text = element_text(size=6),
#                         axis.line.x = element_line(size = 0.3),axis.line.y = element_line(size = 0.3),
#                         axis.ticks.x=element_line(size = 0.3),axis.ticks.y = element_line(size = 0.3),
#                         legend.key.size = unit(0.2, 'cm'),legend.title = element_blank())
...
#               ncol = 4, nrow = 4)
# 
# ggexport(pl2,filename=paste(dir_fig,"/Barplots_charact_sample_red_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
# # scale_y_continuous(limits=c(0,100))
# 
# ggexport(p5,filename=paste(dir_fig,"/Barplots_charact_sample_red_Age_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
# 
...
# # scale_y_continuous(limits=c(0,100))
# 
# ggexport(p6,filename=paste(dir_fig,"/Barplots_charact_sample_red_edu_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
...
p0=ggplot(data=results_plot_MTL2,aes(x = Group_plasmaPET, y = 100*N/n_part,fill=Group)) +
  ...
  geom_text(aes(label=paste(round(100*N/n_part,0),"%",sep = ""),y=10+100*N/n_part),position = position_dodge(.9),size=3)+
...
p1=ggplot(data=results_plot_MTL2,aes(x = Group_plasmaPET, y = 100*Ab/N,fill=Group)) +
  ...
  #               position=position_dodge(.9))+
  ylab("Ab+ (%)") + xlab("Sample reduction groups")+
  geom_text(aes(label=paste(round(100*Ab/N,0),"%",sep = ""),y=5+100*Ab/N),position = position_dodge(.9),size=3)+
  scale_fill_manual(values = c("#d3d3d3",v_color))+
  theme_classic()+theme(legend.position = "none",axis.text=element_text(size=8),#axis.text.x = element_text(angle=45),
                        axis.title=element_text(size=10,face="bold"),legend.text = element_text(size=6),
                        axis.line.x = element_line(size = 0.3),axis.line.y = element_line(size = 0.3),
...
pcost=ggplot(data=results_plot_MTL2[which(results_plot_MTL2$Group=="Pos"),],aes(x = Group_plasmaPET, y = 100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog)),fill=Group)) +
  geom_bar(stat="identity",position="dodge")+theme_classic() +#scale_fill_manual(values=c())+
  geom_errorbar(aes(ymin=100*(USD/(slope-slope_SD/sqrt(N)))/((v_cost_PET*n_part)/mean(df3$slope_cog)), 
                    ymax=100*(USD/(slope+slope_SD/sqrt(N)))/((v_cost_PET*n_part)/mean(df3$slope_cog))), width=.2,position=position_dodge(.9))+
  ylab("USD/slope (%)") + xlab("Sample reduction groups")+
  scale_fill_manual(values = c(v_color))+
  geom_hline(yintercept = 100,linetype="dashed")+#172 number of conversions in the full sample
   geom_text(aes(label=paste(round(100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog)),1),"%",sep = ""),y = 10+100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog))),size=3)+
  theme_classic()+theme(legend.position = "none",axis.text=element_text(size=8),#axis.text.x = element_text(angle=45),
                        axis.title=element_text(size=10,face="bold"),legend.text = element_text(size=6),
                        axis.line.x = element_line(size = 0.3),axis.line.y = element_line(size = 0.3),
                        axis.ticks.x=element_line(size = 0.3),axis.ticks.y = element_line(size = 0.3),
...
#               ncol = 3, nrow = 3)
# 
# ggexport(pl2,filename=paste(dir_fig,"/Barplots_charact_sample_red_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change


pl3=ggarrange(pcost,#plines_part_CSF,
              # labels = c("A", "B", "C","D","E"),
              ncol = 3, nrow = 3)

ggexport(pl3,filename=paste(dir_fig,"/Barplots_Cost_sample_red_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change

pcost2=ggplot(data=results_plot_MTL2[which(results_plot_MTL2$Group=="Pos"),],aes(x = Group_plasmaPET, y = 100-100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog)),fill=Group)) +
  geom_bar(stat="identity",position="dodge")+theme_classic() +#scale_fill_manual(values=c())+
  geom_errorbar(aes(ymin=100-100*(USD/(slope-slope_SD/sqrt(N)))/((v_cost_PET*n_part)/mean(df3$slope_cog)), 
                    ymax=100-100*(USD/(slope+slope_SD/sqrt(N)))/((v_cost_PET*n_part)/mean(df3$slope_cog))), width=.2,position=position_dodge(.9))+
  ylab("US$/slope (%)") + xlab("Sample reduction groups")+
  scale_fill_manual(values = c(v_color))+
  # geom_hline(yintercept = 100,linetype="dashed")+#172 number of conversions in the full sample
  geom_text(aes(label=paste(round(100-100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog)),1),"%",sep = ""),y = 10+100-100*(USD/slope)/((v_cost_PET*n_part)/mean(df3$slope_cog))),size=3)+
  theme_classic()+theme(legend.position = "none",axis.text=element_text(size=8),#axis.text.x = element_text(angle=45),
                        axis.title=element_text(size=10,face="bold"),legend.text = element_text(size=6),
                        axis.line.x = element_line(size = 0.3),axis.line.y = element_line(size = 0.3),
                        axis.ticks.x=element_line(size = 0.3),axis.ticks.y = element_line(size = 0.3),
...
pl4=ggarrange(pcost2,#plines_part_CSF,
              # labels = c("A", "B", "C","D","E"),
              ncol = 4, nrow = 4)

ggexport(pl4,filename=paste(dir_fig,"/Barplots_SavedCost_sample_red_summary_",c_out[i_out],"_DX_",l_DX,"_Ab_",l_ab,"_",l_out,"_",l_PET,"_cost_1_",l_ratio,".pdf",sep = "")) #Do not change
